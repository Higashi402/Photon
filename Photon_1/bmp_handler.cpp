...
#pragma omp parallel for
    for (int y = 0; y < bmp->height; ++y) {
        // Выделение памяти под суммы яркостей
        int* sum_r = (int*)calloc(bmp->width, sizeof(int));
        int* sum_g = (int*)calloc(bmp->width, sizeof(int));
        int* sum_b = (int*)calloc(bmp->width, sizeof(int));

        // Инициализация сумм для первого окна
        for (int x = 0; x < bmp->width; ++x) {
        ...
        // Расчет срединного значения для первого пикселя в строке
        for (int x = 0; x < bmp->width; ++x) {
            int r = sum_r[x] / area;
            int g = sum_g[x] / area;
            int b = sum_b[x] / area;
            filtered_bmp->pixels[y * bmp->width + x] = { (unsigned char)r, (unsigned char)g, (unsigned char)b, 255 };
        }

        // Сдвиг окна и обновление сумм для оставшихся пикселей строки
        for (int x = 1; x < bmp->width; ++x) {
            for (int ky = 0; ky < k; ++ky) {
                pixel p_old = padded_bmp->pixels[(y + ky) * new_width + (x - 1)];
                pixel p_new = padded_bmp->pixels[(y + ky) * new_width + (x - 1 + k)];
                sum_r[x] = sum_r[x - 1] - p_old.red + p_new.red;
                sum_g[x] = sum_g[x - 1] - p_old.green + p_new.green;
                sum_b[x] = sum_b[x - 1] - p_old.blue + p_new.blue;
            }
